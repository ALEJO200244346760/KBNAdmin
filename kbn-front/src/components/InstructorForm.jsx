...
const InstructorForm = () => {
  ...
  const cargar = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [rAgenda, rPasivos] = await Promise.all([
        api.get('/api/agenda/listar'),
        api.get('/api/pasivos'),
      ]);

      // Solo las clases de este instructor
      ...
      // Buscar su tarjeta de pasivo
      const miPasivo = rPasivos.data.find(p => {
        const norm = (s) => (s||'').toLowerCase().replace(/\s+/g,' ').trim();
        return norm(p.titulo) === norm(nombreCompleto);
      });
      setPasivo(miPasivo || null);
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 12px 100px', fontFamily: 'system-ui, sans-serif', background: NA.bg, minHeight: '100dvh' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <Stat icon="ti-clock"    value={horasMes}    label="hs este mes"  />
        <Stat icon="ti-calendar" value={clasesActivas.filter(a => a.fecha?.toString() >= toYMD(inicioMes)).length} label="clases mes" />
        {Object.entries(saldos).map(([m, v]) => (
          <Stat key={m} icon="ti-wallet"
            value={`${m === 'BRL' ? 'R$' : m === 'EUR' ? '€' : 'US$'} ${Math.abs(v).toFixed(0)}`}
            label={v < 0 ? 'te deben' : 'saldo'}
            color={v < 0 ? '#B91C1C' : NA.dark}
            bg={v < 0 ? '#FEE2E2' : NA.light}
          />
        ))}
      </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          {pasivo ? (
            <div style={{ background: NA.darker, borderRadius:16, padding:18 }}>
              <p style={{ margin:'0 0 14px', fontSize:11, fontWeight:700, color:'rgba(255,255,255,.5)', textTransform:'uppercase', letterSpacing:'.06em' }}>
                Cuenta corriente
              </p>
              {Object.entries(saldos).length === 0 ? (
                <p style={{ color:'rgba(255,255,255,.4)', fontSize:13 }}>Sin movimientos registrados.</p>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {Object.entries(saldos).map(([moneda, val]) => {
                    const esNeg = val < -0.001;
                    const simbol = moneda==='BRL'?'R$':moneda==='EUR'?'€':'US$';
                    return (
                      <div key={moneda} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:'rgba(255,255,255,.08)', borderRadius:10, padding:'12px 14px' }}>
                        <div>
                          <p style={{ margin:0, fontSize:11, color:'rgba(255,255,255,.5)' }}>{esNeg ? 'Te deben' : 'Tu saldo'}</p>
                          <p style={{ margin:'2px 0 0', fontSize:11, color:'rgba(255,255,255,.5)' }}>{moneda}</p>
                        </div>
                        <span style={{ fontSize:24, fontWeight:800, color: esNeg ? '#FCA5A5' : '#6EE7B7' }}>
                          {simbol} {Math.abs(val).toFixed(2)}
                        </span>
                      </div>
                    );
                  })}
                  <p style={{ fontSize:11, color:'rgba(255,255,255,.35)', textAlign:'center', margin:'4px 0 0' }}>
                    Las horas de cada clase se acumulan cuando se confirma la clase.
                  </p>
                </div>
              )}
            </div>
          ) : (
            <div style={{ background:'#fff', borderRadius:16, border:`0.5px solid ${NA.border}`, padding:18, textAlign:'center', color:NA.text2 }}>
              <i className="ti ti-wallet-off" style={{ fontSize:28, opacity:.25, display:'block', marginBottom:8 }}/>
              <p style={{ fontSize:13 }}>No tenés tarjeta de cuenta corriente asignada aún.</p>
            </div>
          )}
        </div>
      )}
